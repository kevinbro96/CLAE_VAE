...
parser.add_argument('--eps', default=1.0, type=float, help='eps for adversarial')
...
def gen_adv(net, vae, x1, x2):
    with torch.no_grad():
        z1, gx1, _, _ = vae(x1)
        z2, gx2, _, _ = vae(x2)

    variable_bottle1 = Variable(z1.detach(), requires_grad=True)
    adv_gx1 = vae(variable_bottle1, True)
    inputs_adv1 = adv_gx1 + (x1 - gx1).detach()

    variable_bottle2 = Variable(z2.detach(), requires_grad=True)
    adv_gx2 = vae(variable_bottle2, True)
    inputs_adv2 = adv_gx2 + (x2 - gx2).detach()
    # generate adversarial example
    tmp_loss = CL(net, inputs_adv1, inputs_adv2, gen_adv=True)
    tmp_loss.backward()

    with torch.no_grad():
        sign_grad1 = variable_bottle1.grad.data.sign()
        variable_bottle1 = variable_bottle1 + args.eps * sign_grad1
        adv_gx1 = vae(variable_bottle1, True)
        inputs_adv1 = adv_gx1 + (x1 - gx1).detach()

        sign_grad2 = variable_bottle2.grad.data.sign()
        variable_bottle2 = variable_bottle2 + args.eps * sign_grad2
        adv_gx2 = vae(variable_bottle2, True)
        inputs_adv2 = adv_gx2 + (x2 - gx2).detach()
    inputs_adv1.detach()
    inputs_adv2.detach()
    return inputs_adv1, inputs_adv2, gx1


def CL(model, x1, x2, adv=False, gen_adv=False, index=1):
    if gen_adv:
        x1_feat = model(x1, adv=True)
        x2_feat = model(x2, adv=True)
    elif index == 1:
        x1_feat = model(x1, adv=adv)
        x2_feat = model(x2)
    else:
        x1_feat = model(x1)
...
if args.adv:
    net = models.__dict__[args.resnet + '_cifar'](pool_len=pool_len, low_dim=args.low_dim, bn_adv_flag=True,
...
def write_images(epoch, net, vae, trainloader, batch_num=1, batch_size=64):
    for batch_idx, (inputs1, inputs2, _, indexes) in enumerate(trainloader):
        if batch_idx >= batch_num:
            ...
            if args.adv:
                inputs_adv, _, gx = gen_adv(net, vae, inputs1, inputs2)
                grid_X = torchvision.utils.make_grid(inputs1[:batch_size].data, nrow=8, padding=2, normalize=True)
                ...
                grid_AdvX = torchvision.utils.make_grid((inputs_adv[:batch_size]).data, nrow=8, padding=2,
                                                        normalize=True)
                wandb.log({"_Batch_{batch}_AdvX.jpg".format(batch=batch_idx): [
                    wandb.Image(grid_AdvX)]}, commit=False)
                grid_Delta = torchvision.utils.make_grid(((inputs_adv - inputs1)[:batch_size]).data, nrow=8, padding=2,
                                                         normalize=True)
                wandb.log({"_Batch_{batch}_Delta.jpg".format(batch=batch_idx): [wandb.Image(grid_Delta)]}, commit=False)
                grid_RX = torchvision.utils.make_grid((inputs1 - gx)[:batch_size].data, nrow=8, padding=2,
                                                      normalize=True)
                wandb.log({"_Batch_{batch}_RX.jpg".format(batch=batch_idx): [wandb.Image(grid_RX)]}, commit=False)
                wandb.log({'l2_norm': torch.mean((inputs_adv - inputs1).reshape(inputs1.shape[0], -1).norm(dim=1)),
                           'linf_norm': torch.mean(
                               (inputs_adv - inputs1).reshape(inputs1.shape[0], -1).abs().max(dim=1)[0])})
...
def train(epoch):
    ...
    for batch_idx, (inputs1, inputs2, _, indexes) in enumerate(trainloader):
        ...
        if args.adv:
            inputs_adv1, inputs_adv2, _ = gen_adv(net, vae, inputs1, inputs2)

        optimizer.zero_grad()
        loss_og = CL(net, inputs1, inputs2)
        loss = loss_og
        if args.adv:
            loss_adv = 0.5*CL(net, inputs_adv1, inputs2, adv=True, index=1) + 0.5*CL(net, inputs1, inputs_adv2, adv=True, index=2)
...
for epoch in range(start_epoch, start_epoch + 301):
    ...
    if epoch % test_epoch == 0:
        net.eval()
        write_images(epoch, net, vae, trainloader)
        print('----------Evaluation---------')
        start = time.time()
